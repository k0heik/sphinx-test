...
@cli.command(
    help='train cpc prediction model',
    name='train')
@argument('date')
def train(date: str):
    ...
    df_placements = bq.extract_to_df(
        "./sql",
        "extract_placements.tpl.sql",
        params,
    )
    df["date"] = pd.to_datetime(df["date"])
    df_placements["date"] = pd.to_datetime(df_placements["date"])
    ...
    write_df_to_s3(
        df,
        BUCKET,
        get_s3_key("cpc/extract/", today, "train_data.csv")
    )
    write_df_to_s3(
        df_ad,
        BUCKET,
        get_s3_key("cpc/extract/", today, "train_ad_data.csv")
    )
    write_df_to_s3(
        df_placements,
        BUCKET,
        get_s3_key("cpc/extract/", today, "train_data_placements.csv")
    )
    ...
    df.drop(['current_bidding_price'], axis=1, inplace=True)
    ...
    CPCPredictionService(
        preprocessor=CPCPreprocessor(
            label_encoder_writer=write_latest_labelencoder_manager(
                BUCKET,
                "cpc/model/latest_le/",
                f"cpc/preprocess/{today.year}/{today.month:02}/{today.day:02}/le/"
            ),
            label_encoder_reader=None,
            output=output_preprocess_to_s3_manager(
                BUCKET, f"cpc/preprocess/{today.year}/{today.month:02}/{today.day:02}/train_data.csv",
            ),
            is_use_dask=True,
        ...
        estimator=CPCEstimator(
            model_writer=write_latest_model_manager(
                BUCKET,
                get_s3_key("cpc/model/", today, "model.bin"),
                "cpc/model/latest.bin",
            ),
            is_tune=True,
        ),
    ).train(df, df_placements)
