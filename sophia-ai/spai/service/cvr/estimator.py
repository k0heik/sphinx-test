...
from spai.ai.boosting import ProbModel, get_tuned_params
from .config import (
    FEATURE_COLUMNS,
    TARGET_COLUMN,
    CATEGORICAL_COLUMNS,
    ...
    THRESHOLD_OF_CV_MONTHLY
)


class CVRModel(ProbModel):
    pass
...
class CVREstimator:
    ...
    def _cutoff(self, df: pd.DataFrame) -> pd.DataFrame:
        df = df[(df['ad_id_weekly_sum_clicks'] > THRESHOLD_OF_CLICKS_WEEKLY) &
                (df['ad_id_monthly_sum_conversions'] > THRESHOLD_OF_CV_MONTHLY)]
        return df
    ...
    def fit(self, df: pd.DataFrame):
        ...
        if self._is_tune:
            ...
            params = {
                "params": {
                    "learning_rate": 0.01,
                    "objective": CVRModel.objective,
                    "metric": CVRModel.objective,
                    "verbose": -1,
                    "num_leaves": 15,
                },
                "num_boost_round": 30,
    ...
    def predict(self, df: pd.DataFrame, today):
        ...
        if callable(self._model_reader):
            ...
            model = CVRModel.from_bytes(binary)
        else:
            model = self._model
